...
kfusion::KinFuParams kfusion::KinFuParams::default_params()
{
    ...
    p.cols = 848; //640 848 //pixels
    p.rows = 480;  //pixels
    //p.intr = Intr(525.f, 525.f, p.cols/2 - 0.5f, p.rows/2 - 0.5f); // kinect 1
    p.intr = Intr(423.797f, 423.797f, 426.692, 241.211); //realsense 


    p.volume_dims = Vec3i::all(512);  //number of voxels
    p.volume_size = Vec3f::all(1.f);  //meters
    p.volume_pose = Affine3f().translate(Vec3f(-p.volume_size[0]/2, -p.volume_size[1]/2, 0.3f));

    p.bilateral_sigma_depth = 0.02f;  //meter
    ...
    p.tsdf_trunc_dist = 0.02f; //meters;
...
void kfusion::KinFu::allocate_buffers()
{
    ...
    std::cout << "params_.rows = " << params_.rows << std::endl;
    std::cout << "params_.cols = " << params_.cols << std::endl;
}
