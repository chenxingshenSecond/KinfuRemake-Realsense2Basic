...
namespace 
{
    ...
    {
        std::cout<<std::endl << "prop.major = " << major << " prop.minor = " << minor << std::endl;
        ...
        SMtoCores gpuArchCoresPerSM[] =  { { 0x10,  8 }, { 0x11,  8 }, { 0x12,  8 }, { 0x13,  8 }, { 0x20, 32 }, { 0x21, 48 }, { 30, 64 }, {0x30, 192}, {0x35, 192}, {0x50, 128}, {0x52, 128}, { -1, -1 }  };

        int index = 0;
        while (gpuArchCoresPerSM[index].SM != -1) 
        {
            if (gpuArchCoresPerSM[index].SM == 30 ) { 
                return gpuArchCoresPerSM[index].Cores;
            }
            std::cout << " ((major << 4) + minor)= "<< ((major << 4) + minor) << std::endl;
            index++;
        }
