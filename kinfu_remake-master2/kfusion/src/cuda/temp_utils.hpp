        ...
        struct Block
        {
            ...
            static __kf_device__ T reduce(volatile T* buffer, T init, BinOp op)
            {
                ...
                if (tid < 32)
                {
                    ...
                    if (CTA_SIZE >=   32) val = op(val, __shfl_xor_sync(val, 16));
                    if (CTA_SIZE >=   16) val = op(val, __shfl_xor_sync(val,  8));
                    if (CTA_SIZE >=    8) val = op(val, __shfl_xor_sync(val,  4));
                    if (CTA_SIZE >=    4) val = op(val, __shfl_xor_sync(val,  2));
                    if (CTA_SIZE >=    2) buffer[tid] = op(val, __shfl_xor_sync(val,  1));
#else
                    if (CTA_SIZE >=   64) { buffer[tid] = val = op(val, buffer[tid +  32]); }
                    if (CTA_SIZE >=   32) { buffer[tid] = val = op(val, buffer[tid +  16]); }
                    if (CTA_SIZE >=   16) { buffer[tid] = val = op(val, buffer[tid +   8]); }
