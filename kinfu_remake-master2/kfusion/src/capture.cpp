...
using namespace std;
//using namespace xn;Major revision number
...
"<Configuration>"
"<MapOutputMode xRes=\"848\" yRes=\"480\" FPS=\"30\"/>"
"<Mirror on=\"false\"/>"
"</Configuration>"
"</Node> "
...
void kfusion::OpenNISource::open(int device)
{
    ...
    */
    ...
    std::cout << "intrisic.fx =" << intrinsics.fx << std::endl;
    std::cout << "intrisic.fy =" << intrinsics.fy << std::endl;
    std::cout << "intrisic.ppx =" << intrinsics.ppx << std::endl;
    std::cout << "intrisic.ppy =" << intrinsics.ppy << std::endl;
...
{
	rs2::frameset frameset = pipe.wait_for_frames();
    if (true) {  // Align all frames to depth viewport
        frameset = align_to_depth.process(frameset);
    } else { 
        frameset = align_to_color.process(frameset); // Align all frames to color viewport
    }
    // With the aligned frameset we proceed as usual
    auto depth_ = frameset.get_depth_frame();
    auto color_ = frameset.get_color_frame();
   // auto colorized_depth = c.colorize(depth);

    
    // Query frame size (width and height)
    const int w = depth_.as<rs2::video_frame>().get_width();
    const int h = depth_.as<rs2::video_frame>().get_height();
	...
	if (impl_->has_depth) {
		cv::Mat image(cv::Size(w, h), CV_16U, (void*)depth_.get_data(), cv::Mat::AUTO_STEP);
        image.copyTo(depth); 
	} else {
		depth.release();
		printf("no depth\n");
	...
	if (impl_->has_image) {
        cv::Mat image(cv::Size(w, h), CV_8UC3, (void*)depth_.get_data(), cv::Mat::AUTO_STEP);
        image.copyTo(image); 
	} else {
        image.release();
		printf("no image\n");
